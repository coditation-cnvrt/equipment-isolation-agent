...
def normalized_plan_content(request: dict, result: dict) -> dict:
    ...
    points = [
        _normalized_point(
            item,
            feedback_state=_result_point_feedback_state(item, validation),
        )
        for item in (data.get("isolation_points") or [])
    ]
...
def _normalized_point(item: dict, *, feedback_state: str | None = None) -> dict:
    ...
    return {
        **jsonable(item),
        ...
        "feedback_state": feedback_state or item.get("feedback_state") or "unknown",
    }


def _result_point_feedback_state(item: dict, validation: dict) -> str:
    if (
        item.get("available_for_isolation") is False
        or str(item.get("availability_status") or "").lower() == "unavailable"
    ):
        return PointFeedbackState.UNAVAILABLE.value
    candidate_id = str(
        item.get("uuid")
        or item.get("candidate_id")
        or item.get("drawing_entity_id")
        or ""
    )
    manual_ids = {str(value) for value in validation.get("manual_review_candidate_ids") or []}
    accepted_ids = {
        str(value)
        for key in ("barrier_candidate_ids", "positive_candidate_ids")
        for value in validation.get(key) or []
    }
    if candidate_id in manual_ids:
        return PointFeedbackState.MANUAL_REVIEW.value
    if candidate_id in accepted_ids:
        return PointFeedbackState.ACCEPTED.value
    return PointFeedbackState.EXCLUDED.value
