...
def source_defect_event_stream(
    repository,
    context: dict[str, str],
    *,
    last_event_id: str = "",
    ...
    """Stream durable drawing-scoped source-defect lifecycle events."""

    requested_cursor = str(last_event_id or "").strip()
    if hasattr(repository, "source_defect_event_replay_state"):
        replay = repository.source_defect_event_replay_state(context, requested_cursor)
        cursor = str(replay.get("cursor_id") or "")
        cursor_occurred_at = replay.get("cursor_occurred_at")
        seen_ids = set(replay.get("seen_ids") or set())
    else:
        cursor = requested_cursor or repository.latest_source_defect_event_id(context)
        cursor_occurred_at = None
        seen_ids = {cursor} if cursor else set()
    yield sse_frame("ready", {"kind": "ready", "context": context, "last_event_id": cursor or None})
    last_output = time.monotonic()
    while True:
        rows = repository.list_source_defect_events(context, after_id=cursor, exclude_ids=seen_ids)
        for row in rows:
            event_id = str(row["event_id"])
            if event_id in seen_ids:
                continue
            seen_ids.add(event_id)
            occurred_at = row.get("occurred_at")
            if cursor_occurred_at is None or (occurred_at is not None and occurred_at >= cursor_occurred_at):
                cursor = event_id
                cursor_occurred_at = occurred_at
            yield sse_frame("source_data_defect.changed", row, event_id=event_id)
            last_output = time.monotonic()
        if time.monotonic() - last_output >= heartbeat_interval:
            yield ": heartbeat\n\n"
            last_output = time.monotonic()
