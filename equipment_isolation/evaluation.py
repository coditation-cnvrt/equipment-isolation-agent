...
SCOPE / WHAT THIS DOES NOT CHECK -- read before trusting a green result:
  - BOTH sides run through ``equipment_isolation.agent.tools`` on a BARE ``RunConfig`` (see
    ...
    exercise the deterministic or agentic CLI paths, and it
    ...
    ``equipment-isolation-agent`` output against ``equipment-isolation`` directly.
...
Usage:
    uv run equipment-isolation-eval BT-11 BT-12
    uv run equipment-isolation-eval --limit 5        # first 5 equipment tags
"""
...
from equipment_isolation.integrations.graph_client import GraphClient, normalize_vertex, props_only
...
def build_config(equipment_tag: str) -> RunConfig:
    ...
    # reflect the runner / agent CLI paths (see module docstring). To make this
    # a real parity check, apply_project_profile + resolve_project_metadata would
    # need to run here, matching the shared CLI config builder.
    return RunConfig(
