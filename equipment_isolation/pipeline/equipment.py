...
from equipment_isolation.integrations.graph_client import GraphClient, normalize_vertex, props_only, vertex_id
from equipment_isolation.pipeline.job_inference import _first_value, _norm
...
def list_equipment(graph_config, limit=0):
    ...
    for row in rows:
        ...
        pnid_reference = _first_value(props, ("pnid", "pnid_id", "job_id"))
        job_id = _job_id_from_pnid_reference(pnid_reference)
        job_name = _first_value(props, ("job_name", "pnid_name", "drawing_name"))
        items.append(
            {
                "id": str(vertex_id(row)),
                "tag": tag or "",
                "name": name or "",
...
def _job_id_from_pnid_reference(value):
    text = str(value or "").strip()
    if not text:
        return ""
    marker = "pnid:job:"
    return text[len(marker):].strip() if text.lower().startswith(marker) else text
...
def add_equipment_jobs_from_metadata(items, job_ids_by_name):
    ...
    by_id = {str(job_id): (str(name), str(job_id)) for name, job_id in job_ids_by_name.items()}
    for item in items:
        job = by_id.get(str(item.get("job_id") or "")) or by_name.get(str(item.get("job_name") or "").strip().lower())
        if job:
            item["job_name"] = job[0]
            item["job_id"] = job[1]
    return items
